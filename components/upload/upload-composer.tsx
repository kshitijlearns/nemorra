...
import { encodeFile, learningRequest, materialSchema, type Material } from '@/lib/learning';
import { getCloudUserId } from '@/lib/cloud';
import { uploadToS3 } from '@/lib/s3-upload';

const maxCloudFileBytes = 15 * 1024 * 1024;
...
export function UploadComposer() {
  ...
  async function beginSession(material: Material) {
    try {
      const planned = materialSchema.parse(await learningRequest({ action: 'memory-plan', material }));
      startSession(planned);
    } catch {
      // The Learn screen can retry the memory map; never discard successfully extracted material.
  ...
  async function submit() {
    ...
    if (file && file.size > maxCloudFileBytes) { setError('Choose a file that is 15 MB or smaller.'); return; }
    ...
    try {
      ...
      if (file) {
        if (file.name.toLowerCase().endsWith('.txt')) text = [notes, await file.text()].filter(Boolean).join('\n\n');
          ...
          if (cloudUpload) {
            const response = await fetch('/api/learning/from-s3', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ userId: getCloudUserId(), key: cloudUpload.key, notes }),
            });
            const data = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(data.error || 'Could not process the uploaded file.');
            await beginSession(materialSchema.parse(data));
            return;
          }
      ...
      if (text.length > 20_000) throw new Error('Use at most 20,000 characters. Split longer notes into smaller lessons.');
      const material = materialSchema.parse(await learningRequest({ action: 'prepare', text, file: attachment }));
      await beginSession(material);
    } catch (e) { setError(e instanceof Error ? e.message : 'Could not process this material.'); }
    finally { lock.current = false; setBusy(false); }
  }

  return <><div className="upload-box"><textarea aria-label="Learning notes" placeholder="Add your notes, PDF, image..." rows={3} maxLength={20_000} value={notes} disabled={busy} onChange={e => setNotes(e.target.value)} />{file && <div className="attachment-label"><FileText size={15}/><span className="truncate">{file.name}</span><button disabled={busy} aria-label="Remove attachment" onClick={() => setFile(null)}><X size={16}/></button></div>}<div className="upload-controls"><button disabled={busy} className="icon-button outlined" aria-label="Add document or image" onClick={() => input.current?.click()}><Plus size={25}/></button><div className="flex items-center gap-2"><button disabled={busy} className="pill-button" onClick={() => setSample(true)}>Try a sample</button><button className="icon-button solid" aria-label="Start learning from notes" disabled={busy || (!file && notes.trim().length < 20)} onClick={submit}><ArrowUp size={26}/></button></div></div><input ref={input} type="file" className="sr-only" tabIndex={-1} accept=".txt,.pdf,image/jpeg,image/png,image/webp" onChange={e => { setFile(e.target.files?.[0] || null); setError(''); e.target.value = ''; }}/></div>{busy && <p role="status" className="quiet-note mt-3">Reading your material and building a memory map…</p>}{error && <p role="alert" className="text-destructive text-sm mt-3">{error}</p>}<p className="quiet-note mt-3">PDF, TXT, JPG, PNG, WebP. Cloud mode uploads documents to private S3 before Gemini processing. Files up to 15 MB.</p><SampleModal open={sample} onClose={() => setSample(false)}/></>;
}
