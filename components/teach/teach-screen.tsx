...
export function TeachScreen() {
  ...
  return <PageTransition><div className="teach-topbar"><div className="teach-top-left"><Link href="/" className="icon-button outlined" aria-label="Back home"><ArrowLeft size={19}/></Link><Link href="/write" className="teach-write-button" aria-label="Open Write"><Pencil size={14}/><span>Write</span></Link></div><button className="icon-button" aria-label="Review learning material" onClick={() => setReview(true)}><BookOpen size={22}/></button></div><h1 className="page-title">Teach</h1><p className="page-subtitle mt-3">Explain it in your own words. I’m listening.</p><p className="quiet-note mt-3">{session.material.title}</p>
